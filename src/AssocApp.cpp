...
public:
    ...
    bool frameRenderingQueued(const Ogre::FrameEvent& evt) override
    {
        ...
        {
            const Ogre::Real phase = mT * (0.38f + 0.04f * static_cast<Ogre::Real>(i)) + static_cast<Ogre::Real>(i);
            Ogre::SceneNode* const node = mSmokeNodes[i];
            node->setPosition(mSmokeBase[i] + Ogre::Vector3(Ogre::Math::Sin(phase) * 3.5f,
                Ogre::Math::Sin(phase * 0.7f) * 1.1f, Ogre::Math::Cos(phase * 0.9f) * 4.5f));
            node->yaw(Ogre::Degree(Ogre::Math::Sin(phase) * 0.35f));
            node->setScale(1.0f + 0.08f * Ogre::Math::Sin(phase), 1.0f + 0.06f * Ogre::Math::Cos(phase * 1.2f),
                1.0f + 0.12f * Ogre::Math::Sin(phase * 0.8f));
        }

        for (std::size_t i = 0; i < mBeamNodes.size(); ++i)
        {
            const Ogre::Real shimmer = 1.0f + 0.03f * Ogre::Math::Sin(mT * 1.7f + static_cast<Ogre::Real>(i));
...
private:
    ...
    void createMaterials()
    {
        ...
        makeMaterial("assoc/projectorBody", Ogre::ColourValue(0.03f, 0.035f, 0.04f, 1.0f));
        makeMaterial("assoc/projectorLens", Ogre::ColourValue(0.72f, 0.88f, 1.0f, 0.78f),
            Ogre::ColourValue(0.18f, 0.34f, 0.48f), true);
        makeMaterial("assoc/beamWarm", Ogre::ColourValue(1.0f, 0.72f, 0.35f, 0.18f),
            Ogre::ColourValue(0.22f, 0.13f, 0.04f), true);
        makeMaterial("assoc/beamCool", Ogre::ColourValue(0.55f, 0.82f, 1.0f, 0.16f),
            Ogre::ColourValue(0.06f, 0.12f, 0.22f), true);
        makeMaterial("assoc/smoke", Ogre::ColourValue(0.78f, 0.82f, 0.86f, 0.18f),
            Ogre::ColourValue(0.04f, 0.05f, 0.06f), true);
    }
    ...
    void buildAtmospherics()
    {
        const Ogre::Vector3 target{0.0f, 14.0f, -20.0f};
        const std::array<Ogre::Vector3, 3> projectorPos{{
            {-118.0f, 19.0f, 72.0f},
            {118.0f, 20.0f, 70.0f},
            {0.0f, 27.0f, 130.0f},
        }};
        const std::array<Ogre::String, 3> beamMats{{"assoc/beamWarm", "assoc/beamCool", "assoc/beamWarm"}};
        const std::array<Ogre::ColourValue, 3> lightColours{{
            Ogre::ColourValue(1.0f, 0.72f, 0.38f, 1.0f),
            Ogre::ColourValue(0.52f, 0.78f, 1.0f, 1.0f),
            Ogre::ColourValue(0.95f, 0.9f, 0.68f, 1.0f),
        }};

        for (std::size_t i = 0; i < projectorPos.size(); ++i)
        {
            Ogre::SceneNode* stand = mScene->getRootSceneNode()->createChildSceneNode(
                "projectorStand" + Ogre::StringConverter::toString(i), projectorPos[i] + Ogre::Vector3(0.0f, -8.0f, 0.0f));
            stand->attachObject(createBox(mScene, "projectorStandBox" + Ogre::StringConverter::toString(i),
                Ogre::Vector3(2.0f, 16.0f, 2.0f), "assoc/carbon"));
            ...
            projector->attachObject(createBox(mScene, "projectorBodyBox" + Ogre::StringConverter::toString(i),
                Ogre::Vector3(13.0f, 7.0f, 10.0f), "assoc/projectorBody"));

            Ogre::SceneNode* lens = projector->createChildSceneNode(
                "projectorLens" + Ogre::StringConverter::toString(i), Ogre::Vector3(0.0f, 0.0f, -5.6f));
            lens->attachObject(createBox(mScene, "projectorLensBox" + Ogre::StringConverter::toString(i),
                Ogre::Vector3(7.5f, 4.6f, 0.7f), "assoc/projectorLens"));

            Ogre::SceneNode* beam = projector->createChildSceneNode(
                "projectorBeam" + Ogre::StringConverter::toString(i), Ogre::Vector3(0.0f, 0.0f, -6.0f));
            beam->attachObject(createBeam(mScene, "projectorBeamMesh" + Ogre::StringConverter::toString(i),
                beamMats[i], 118.0f, 2.4f, 24.0f));
            mBeamNodes.push_back(beam);

            Ogre::Light* spot = mScene->createLight("projectorSpot" + Ogre::StringConverter::toString(i));
            spot->setType(Ogre::Light::LT_SPOTLIGHT);
            spot->setDiffuseColour(lightColours[i]);
            spot->setSpecularColour(lightColours[i]);
            spot->setSpotlightRange(Ogre::Degree(10.0f), Ogre::Degree(24.0f), 0.75f);
            spot->setAttenuation(190.0f, 1.0f, 0.004f, 0.0f);
            Ogre::SceneNode* lightNode = projector->createChildSceneNode(
                "projectorSpotNode" + Ogre::StringConverter::toString(i), Ogre::Vector3(0.0f, 0.0f, -5.8f));
            lightNode->attachObject(spot);
        }

        Ogre::SceneNode* smokeMachine = mScene->getRootSceneNode()->createChildSceneNode("smokeMachine",
            Ogre::Vector3(-44.0f, 4.0f, 42.0f));
        smokeMachine->yaw(Ogre::Degree(-18.0f));
        smokeMachine->attachObject(createBox(mScene, "smokeMachineBox", Ogre::Vector3(20.0f, 8.0f, 12.0f), "assoc/projectorBody"));
        Ogre::SceneNode* smokeNozzle = smokeMachine->createChildSceneNode("smokeNozzle", Ogre::Vector3(10.5f, 1.0f, 0.0f));
        smokeNozzle->attachObject(createBox(mScene, "smokeNozzleBox", Ogre::Vector3(3.0f, 2.4f, 5.0f), "assoc/carbon"));

        for (int i = 0; i < 14; ++i)
        {
            const Ogre::Vector3 base{-26.0f + static_cast<Ogre::Real>(i % 5) * 14.0f,
                8.5f + static_cast<Ogre::Real>(i % 4) * 4.0f,
                22.0f - static_cast<Ogre::Real>(i) * 7.0f};
            Ogre::SceneNode* smoke = mScene->getRootSceneNode()->createChildSceneNode(
                "smokePuff" + Ogre::StringConverter::toString(i), base);
            ...
            smoke->attachObject(createBox(mScene, "smokePuffBox" + Ogre::StringConverter::toString(i),
                Ogre::Vector3(24.0f + static_cast<Ogre::Real>(i % 3) * 7.0f, 5.0f, 11.0f), "assoc/smoke"));
